...
function EditArticle({ data }: any) {
    //const [article, setArticle] = useState<Article>(data as Article);
    const [article, setArticle] = useState<Article>({
        articleId: data.articleId,
        articleHeader: data.articleHeader,
        articleSubHeader: data.articleSubHeader,
        articleMainImageURL: data.articleMainImageURL,
    ...
    const [imagesMainURL, setImagesMainURL] = useState<string[]>(data.articleMainImageURL);
    const [imageMainURLText, setImageMainURLText] = useState<string>('');

    const [articleImagesURL, setArticleImagesURL] = useState<string[]>(data.articleImageURL);
    const [articleImageURLText, setArticleImageURLText] = useState<string>('');

    const [articleParagraphs, setArticleParagraphs] = useState<string[]>(data.articleParagraph);
    ...
    async function editArticle(articleId: number, header: string, subHeader: string, mainImage: string[], paragraph: string[], articleImage: string[]) {
    ...
    const handleOnSubmitEditArticle = async (event: SyntheticEvent) => {
        event.preventDefault();

        Swal.fire({
            title: 'Edit New Article',
            text: "Do you wish to edit the Article?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                const response = await editArticle(article.articleId, article.articleHeader, article.articleSubHeader, imagesMainURL, articleParagraphs, articleImagesURL);
                console.log(response);
                if (response) {
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
                        title: 'Article Updated Succesfully!',
                        showConfirmButton: true,
                    }).then(() => {
        ...
        <Box>
            <Box>
                <Container maxW={'container.xl'} mt='2rem'>
                    <Heading>Edit Article <FaNewspaper></FaNewspaper></Heading>
                    <Divider></Divider>

                    <Box p='5' shadow={'xl'} mt='2rem'>
                        <form onSubmit={handleOnSubmitEditArticle}>
                            <Box mb='2rem'>
                                <FormLabel>Header</FormLabel>
                                <Input value={article.articleHeader} onChange={handleOnChangeHeader}></Input>
                            </Box>

                            <Box mb='2rem'>
                                <FormLabel>Sub Header</FormLabel>
                                <Input value={article.articleSubHeader} onChange={handleOnChangeSubHeader}></Input>
                            </Box>
